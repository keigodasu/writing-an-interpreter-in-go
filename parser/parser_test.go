...
`
		...
		if len(program.Statements) != 3 {
			t.Fatalf("program.SDtatements does not contain 3 statements. got=%d", len(program.Statements))
		}
		...
		for _, stmt := range program.Statements {
			...
			if returnStmt.TokenLiteral() != "return" {
				t.Errorf("returnStmt.TokenLItera not return got %q", returnStmt.TokenLiteral())
			}
		}

	})

	t.Run("LetStatement", func(t *testing.T) {
		input := `
let x = 5;
let y = 10;
let foobar = 838383;
...
func TestExpression(t *testing.T) {
	...
	t.Run("integer literal expression", func(t *testing.T) {
		...
		if literal.Value != 5 {
			t.Errorf("ident.Value not %q. got=%q", "foorbar", literal.Value)
		}
	...
	t.Run("infix expression", func(t *testing.T) {
		...
		for _, tt := range infixTests {
			...
			literal, ok := stmt.Expression.(*ast.InfixExpression)
			if !ok {
				t.Fatalf("exp not *ast.Identifier. got=%T", stmt.Expression)
			}
			if literal.Operator != tt.operator {
			...
			if !testIntegerLiteral(t, literal.Right, tt.rightValue) {
				return
			}
		}
	})
...
func testLetStatement(t *testing.T, s ast.Statement, name string) bool {
	...
	if letStmt.Name.TokenLiteral() != name {
		t.Errorf("letStmet.Name.TokenLitera() not %s. got=%s", name, letStmt.Name.TokenLiteral())
		return false
	}
